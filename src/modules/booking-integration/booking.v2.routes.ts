...
export function createReservaBookingV2Router(reservaRepo: ReservaRepository): Router {
  ...
  // POST /api/v2/emilypamela/reservas/booking
  ...
  // PATCH /api/v2/emilypamela/reservas/booking/:id
...
// ── /api/v2/emilypamela/alquileres/booking ────────────────────────────────────
export function createAlquilerBookingV2Router(alquilerRepo: AlquilerRepository): Router {
  const router = Router();
...
// ── /api/v2/emilypamela/devoluciones/booking ─────────────────────────────────
export function createDevolucionBookingV2Router(alquilerRepo: AlquilerRepository): Router {
  const router = Router();
  ...
  router.post('/', async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      publish('alquileres.devolucion.registrada', devolucion.id, {
        ...
        alquilerId,
        reservaId:   alquiler.reservaId,
        vehiculoId:  reservaObj?.vehiculoId,
        status:      'COMPLETADA',
      });
